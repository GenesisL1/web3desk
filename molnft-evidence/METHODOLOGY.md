...
The capture is intentionally immutable for Article 02: any later block belongs in a new snapshot rather than silently replacing the evidence cited by the article. The committed snapshot is therefore both a research artifact and a versioned citation target. The workflow run itself is retained as an additional execution record.
